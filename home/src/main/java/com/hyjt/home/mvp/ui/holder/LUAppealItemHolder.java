 ...
 * @desc ${TODO}
 */
public class LUAppealItemHolder extends BaseHolder<LUReqsListResp.RowsBean> {
    ...
    public LUAppealItemHolder(View v, String type) {
        super(v);
    ...
    @Override
    public void setData(LUReqsListResp.RowsBean data, int position) {
        rtNum.setText(data.getNumber());
        rtType.setText(data.getState());
        rtTime.setText(data.getAppealTime());

        if ("Mine".equals(type)){
            rtName.setText(data.getBoss());
        } else {
            rtName.setText(data.getAppealPerson());
        }
