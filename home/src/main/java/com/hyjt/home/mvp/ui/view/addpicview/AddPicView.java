...
public class AddPicView extends RelativeLayout implements AddPicCallBack {
    ...
    public AddPicView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        TypedArray vArray = getResources().obtainAttributes(attrs, R.styleable.home_AddPicView);
        int numColumns = vArray.getInteger(R.styleable.home_AddPicView_home_numColumns, 4);
        View vView = LayoutInflater.from(context).inflate(R.layout.home_add_pic_view, this, true);
